...
layout(location=4) uniform int nbp;
...
void srand()
{
	random_seed = (float(nbp+instID)+date)*dot(screen_tc,vec2(12.9898,78.233));
	random_inc = PI/37.0;
}
...
/*
vec3 random_sample_cube(float r)
{
	float ar = DPI * r;
	float x = fract(sin(ar)*43758.5453123);
	float y = fract(sin(1.7*ar)*43758.5453123);
	float z = fract(sin(2.5*ar)*43758.5453123);

	return vec3(x,y,z)*2.0-1.0;
}

vec3 random_sample_oriented_quad(float r)
{
	float ar = DPI * r;
	float x = fract(sin(ar)*43758.5453123);
	float y = fract(sin(1.7*ar)*43758.5453123);
	return vec3(vec2(x,y)*2.0-1.0,0.0);
}

vec3 random_sample_sphere(float r)
{

	float ar = DPI * r;
	float r1 = fract(sin(ar)*43758.5453123);
	float r2 = fract(sin(1.7*ar)*43758.5453123);
	float r3 = fract(sin(2.5*ar)*43758.5453123);
	float h = fract(r1)*2.0-1.0;
	float alpha = asin(h);
	float beta = DPI*r2;
	float rad = cos(alpha)*r3;
	return vec3(rad*cos(beta),rad*sin(beta),h*r3);
}

vec3 random_sample_cylinder(float r)
{
	return vec3(0,0,2.0*r-1.0);

	float ar = DPI * r;
	float rad = abs(fract(sin(ar)*43758.5453123));
	float alpha = DPI * fract(sin(1.5*ar)*43758.5453123);
	float z = fract(sin(2.0*ar)*43758.5453123);
	return vec3(rad*cos(alpha),rad*sin(alpha),z);
}


vec3 random_sample_primitive(int p, float r)
{
	switch(read_primitive_type_simple(p))
	{
		case CODE_SPHERE:
			return random_sample_sphere(r);
			break;
		case CODE_CUBE:
			return random_sample_cube(r);
			break;
		case CODE_ORIENTED_QUAD:
			return random_sample_oriented_quad(r);
			break;
		case CODE_CYLINDER:
			return random_sample_cylinder(r);
			break;
	}
	return vec3(0);
}*/
